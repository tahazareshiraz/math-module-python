...
class TestStatistics(unittest.TestCase):
    ...
    def test_variance_stdev(self):
        self.assertAlmostEqual(tms.variance(self.data), statistics.variance(self.data), places=6)
        self.assertAlmostEqual(tms.stdev(self.data), statistics.stdev(self.data), places=6)
        self.assertAlmostEqual(tms.pvariance(self.data), statistics.pvariance(self.data), places=6)
        self.assertAlmostEqual(tms.pstdev(self.data), statistics.pstdev(self.data), places=6)

    def test_geometric_harmonic_mean(self):
        pos_data = [1, 2, 3, 4, 5]
        self.assertAlmostEqual(tms.geometric_mean(pos_data), statistics.geometric_mean(pos_data), places=4)
        self.assertAlmostEqual(tms.harmonic_mean(pos_data), statistics.harmonic_mean(pos_data), places=6)
